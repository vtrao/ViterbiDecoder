...
//      printf("tails is %d",btd(tail,K-k));
...
void addnoise()                 /* This function actually adds noise to the encoded.txt file, generates gaussop.txt*/
        ...
        int ci;
        char v;
        float snr=6.0,sigma,mean=0,sn_r,es=1;
...
void decode()                      /* This is the actually Decoder of the Project */
        ...
        int cur,t,x1,x2,z=0,*ssa,i=0,j=0;
        int bestinputb,bestinputa,bestinput;                            /* new entry #1*/
        int op,flag=0,flag1=0,minst,*vvv;
	vvv=(int *)malloc(k*sizeof(int));
	ssa=(int *)malloc((limit+1)*sizeof(int));
        float wt,mineu,fl;
        printf("\n");
        for(i=0;i<K-k;i++)
                printf(" %d",tail[i]);
...
//              printf("\n %f",str[i]);
        ...
        for(i=0;i<pow1;i++)
                ...
                if(stprev[i][t].value==-1)
                        ...
                        if(flag==0)
                                {
                                mineu=stprev[i][t].accum;
                                minst=stprev[i][t].prev;
                                flag=1;
                                }
                        else
                                if(mineu>=fl)
        ...
        for(i=limit-1;i>=0;i--)
                {
                ssa[i]=stprev[ssa[i+1]][i+1].prev; /* This is the Back Tracking the Trellis */
                }
        fclose(fp);
        fp=fopen("decoded.txt","a");
	//fputc('\n',fp);
        ssa[0]=cur;
        for(i=0;i<=limit;i++)
                {
...
int main()
	...
	lf=fopen("logfile.txt","w");
        ...
        inputvalidation1:
                ...
                if(k>n)
                        {
                        printf("Invalid input k cant be greater than n\n Re Enter");
